...
import scripts.co_rl.doublebee_mpc as mpc
...
def thrust_to_pwm(T):
    """Invert PWM->thrust polynomial to get PWM for desired thrust T [N]."""
    c = [4.0792540792478203e-13, -2.2921522921483562e-09,
         2.2550699300690226e-05, -0.026882905982896884, 8.516433566430207]
    pwms = np.linspace(1000, 2000, 1001)
    thrusts = np.polyval(c, pwms)
    return float(pwms[np.argmin(np.abs(thrusts - T))])
...
def main():
    ...
    while simulation_app.is_running():
        ...
        try:
            ...
            step_ahead = torch.clamp((max_ahead - ground_z) / 0.04, 0.0, 1.0)
            step_ahead_val = float(step_ahead[0].item())

            # NEW: slow-decay memory of recent step proximity, so yaw authority
            # doesn't collapse right at the crest just because step_ahead reads
            # low between steps or right after clearing one
            if not hasattr(env, "_recent_step_max"):
            ...
            if timestep % 20 == 0:
                print(f"[STEP DETECT] step_ahead={step_ahead_val:.2f} recent_max={env._recent_step_max:.2f} theta_desired={np.degrees(theta_desired):.1f}deg", flush=True)
        except (KeyError, ValueError, IndexError) as e:
            if timestep % 20 == 0:
                print(f"[STEP DETECT ERROR] {e}", flush=True)

        # # --- v_desired / bearing (now step_ahead_val is available) ---
        ...
        #     target_xy = cmd.current_targets_w[0, :2].cpu().numpy()
        #     robot_xy = np.array([pos[0].item(), pos[1].item()])
        #     dist_to_target = target_xy[1] - robot_xy[1]
        #     v_desired = float(np.clip(dist_to_target * 0.15, -0.3, 0.3))

        #     # don't retreat while a step is present — keep pushing forward
        #     if step_ahead_val > 0.1:
        #         v_desired = max(v_desired, 0.15)

        #     to_target = target_xy - robot_xy
        #     bearing_to_target = float(np.arctan2(to_target[0], to_target[1]))

        # # only zero v_desired if theta is large AND unintentional (not actively climbing)
        # # if abs(theta) > np.radians(10) and abs(theta_desired) < np.radians(5):
        ...
        if timestep % 20 == 0:
            print(f"[NAV] dist={dist_to_target:.2f} v_des={v_desired:.2f} "
        ...
        print(f"[SPIN CHECK] yaw={np.degrees(yaw):.1f}deg yaw_rate={ang_vel_yaw:.2f} theta={np.degrees(theta):.1f}deg")

        if timestep % 20 == 0:
            print(f"[DECOUPLED] theta={np.degrees(theta):.1f}deg thetadot={theta_dot:.2f} "
                  f"v={v_fwd:.2f} v_des={v_desired:.2f} -> T={out['T']:.1f}N sigma={np.degrees(out['sigma']):.1f}deg "
                  f"tau_w={out['tau_w']:.2f} action={actions[0].cpu().numpy().round(2)}", flush=True)
        # ============ END DECOUPLED CONTROL ============

        # # ---- TEMP: manual action test, ignore policy/MPC/PID entirely ----
        # robot = env.unwrapped.scene["robot"]
        # lpj = robot.joint_names.index("leftPropeller")
        # rpj = robot.joint_names.index("rightPropeller")
        # prop_vel = robot.data.joint_vel[0, [lpj, rpj]]
        # print(f"[PROP CHECK] left_prop_vel={prop_vel[0].item():.1f} right_prop_vel={prop_vel[1].item():.1f}")

        # # actions = torch.tensor(
        # #     [[0.5, 0.5, 0.0, 0.0, 0.0, 0.0]],   # equal wheel command, nothing else
        # #     device=env.unwrapped.device, dtype=torch.float32,
        # # )
        # actions = torch.tensor(
        #     [[0.5, 0.5, 0.0, 0.0, -1.0, -1.0]],   # -1.0 for props = actually idle (0 rad/s)
        #     device=env.unwrapped.device, dtype=torch.float32,
        # )
        # obs, _, _, extras = env.step(actions)
        # if timestep % 20 == 0:
        ...
        #     print(f"[WHEEL TEST] step={timestep} yaw={np.degrees(yaw):.1f}deg pos_y={robot.data.root_pos_w[0,1].item():.3f}", flush=True)
        # # ---- END TEMP ----
...
# ../../isaaclab/IsaacLab/isaaclab.sh -p scripts/co_rl/play_mpc.py --task Isaac-Velocity-HybridStair-DoubleBee-Play-v1-ppo --num_envs 1 --algo tqc --load_run $RUN --checkpoint model_500.pt
