"""Eval variant for the generalization terrains: adds body pitch and a per-step
trajectory dump for env 0.

SEPARATE FILE from eval_climb.py on purpose. The 250-run height sweep is using
eval_climb.py right now, and swapping a script under a running job is how
today's NO RUN DIR and pinned-terrain failures happened.

--traj_out writes the BEST episode seen, not env 0's first one. Measured
2026-09-10: env 0's opening episode is frequently a short failure -- 0.54 m for
a checkpoint whose mean travel is 1.99 m -- and a "sustained progression" panel
plotting a 0.5 m stub misrepresents the policy in the unflattering direction.
"Best" means reached the goal, tie-broken on distance travelled; if nothing
reaches a goal it falls back to the furthest episode, so descending terrains
(where goal is the only sensible success) still produce a trace.
"""
...
def main():
    ...
    p.add_argument("--traj_out", default=None,
                   help="per-step t,x,y,z,pitch for env 0's first episode")
    ...
    # rolling per-env buffers, plus the best completed episode so far
    ntrack = min(n, 8)
    buf = [[] for _ in range(ntrack)]
    best_traj, best_key = None, (-1, -1.0)   # (reached_goal, distance)
        ...
        with torch.inference_mode():
            ...
            if a.traj_out:
                for kk in range(ntrack):
                    buf[kk].append((len(buf[kk]) * base.step_dt,
                                    float(pos[kk, 0].item() - spawn[kk, 0].item()),
                                    float(pos[kk, 1].item() - spawn[kk, 1].item()),
                                    float(pos[kk, 2].item() - spawn[kk, 2].item()),
                                    float(pitch[kk].item())))
    ...
    if a.traj_out and best_traj:
        with open(a.traj_out, "w", newline="") as tf:
            tw = csv.writer(tf)
            tw.writerow(["t", "x", "y", "z", "pitch_deg"])
            tw.writerows([[round(v, 5) for v in r] for r in best_traj])
        print("wrote %s  (%d steps, %.1f s, %s, %.2f m)"
              % (a.traj_out, len(best_traj), best_traj[-1][0],
                 "goal" if best_key[0] else "no goal", best_key[1]), flush=True)
