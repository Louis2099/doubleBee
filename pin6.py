...
OUT, STEP_H, NENV, TICKS = "simprof/pinned6b.csv", 0.06, 16, 4000

env_cfg = parse_env_cfg(TASK, num_envs=NENV)
tg = env_cfg.scene.terrain.terrain_generator
# Pin ONLY the height range. eval_climb.py also sets curriculum=False and
# num_rows=1, and its own 2026-09-05 note blames the flat-patch sampling that is
# tied to the curriculum tiles -- i.e. the LAYOUT collapse, not the height.
# step_height = min + difficulty*(max-min), so min==max gives 6 cm on every tile
# while rows, columns and patch sampling stay exactly as trained.
tg.use_cache = False
